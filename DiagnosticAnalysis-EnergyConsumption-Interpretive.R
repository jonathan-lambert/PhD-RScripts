...
m2 <- lm(log(energyWh) ~ jvm + gcc + gc, data = data.all.df)
m3 <- lm(log(energyWh) ~ jvm + gcc + gc + category, data = data.all.df)
m4 <- lm(log(energyWh) ~ jvm + gcc + gc + category +
           bmr + l1dlmr + l2dlmr + pfmr, data = data.all.df)

cd <- cooks.distance(m4)
influential <- which(cd > (4/nrow(data.int)))

m4.sensitivity <- lm(
  log(energyWh) ~ jvm + gcc + gc + category + bmr + l1dlmr + l2dlmr + pfmr,
  data = data.int[-influential,]
)
