...
power_formula <- meanW ~ jvm + gcc + mode + gc + category +
  bmr + l1dlmr + l2dlmr + pfmr

power_model <- lm(power_formula, data = data.all.df)
...
knitr::kable(
  ...
  digits = 3,
  row.names = FALSE,
  caption = "Stratified bootstrap coefficient stability analysis for the pooled power use regression model.",
