...
# -----------------------------
# Run Tiered power model
# -----------------------------

power_pooled <- run_power_hierarchy(data.hs, include_mode = FALSE)

m1 <- lm(meanW ~ jvm + gcc, data = data.hs)
m2 <- lm(meanW ~ jvm + gcc + gc, data = data.hs)
m3 <- lm(meanW ~ jvm + gcc + gc + category, data = data.hs)
m4 <- lm(meanW ~ jvm + gcc + gc + category +
