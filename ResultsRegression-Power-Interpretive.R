...
# ---------------------------------------------------
# Hierarchical model function: Interpretive Execution
# ---------------------------------------------------

run_power_hierarchy <- function(data, include_mode = FALSE) {
...
# -----------------------------
# Run Interpretive power model
# -----------------------------

power_pooled <- run_power_hierarchy(data.int, include_mode = FALSE)

m1 <- lm(meanW ~ jvm + gcc, data = data.int)
m2 <- lm(meanW ~ jvm + gcc + gc, data = data.int)
m3 <- lm(meanW ~ jvm + gcc + gc + category, data = data.int)
m4 <- lm(meanW ~ jvm + gcc + gc + category +
           bmr + l1dlmr + l2dlmr + pfmr, data = data.int)
summary(m1)
summary(m2)
summary(m3)
