...
energy_formula <- log(energyWh) ~ jvm + gcc + gc + category +
  bmr + l1dlmr + l2dlmr + pfmr

energy_model <- lm(energy_formula, data = data.int)

# Create strata variable from experimental design cells
data.boot.df <- data.int %>%
  mutate(
...
knitr::kable(
  ...
  digits = 3,
  row.names = FALSE,
  caption = "Stratified bootstrap coefficient stability analysis for the interpretive execution energy consumption regression model.",
