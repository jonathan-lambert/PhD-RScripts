# Interaction Effects: JRE and Workload
# Tables
...
make_two_column_power_table <- function(power.summary.pooled,
  ...
  cat("\\centering\n")
  cat("\\scriptsize\n")
  cat(sprintf("\\caption{Power Use Distributions (%s): Descriptive statistics summarising power use observations across workload categories and JRE JVM versions under %s.}\n",
  ...
  cat("\\textbf{Category} & \\textbf{JRE} & \\textbf{M} & \\textbf{M$_d$} & \\textbf{SD} & \\textbf{IQR}\n")
  cat("&\n")
  cat("\\textbf{Category} & \\textbf{JRE} & \\textbf{M} & \\textbf{M$_d$} & \\textbf{SD} & \\textbf{IQR} \\\\\n")
...
make_two_column_energy_table.interpretive <- function(df,
                                                      heading = "Interpretive Execution (Wh)") {
